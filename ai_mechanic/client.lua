...
RegisterCommand("mecanico", function(source, args)   -- Modify the "mechanic" value to change activation command.
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, target)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "Mecanica", "Um mecânico foi enviado para o seu local. Obrigado por usar ~y~" .. companyName)
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(target, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Veiculo Reparado" , mechPedPick.lines[math.random(#mechPedPick.lines)])
...
function GetTargetVehicle(player, dir)
    ...
    if DoesEntityExist(dmgVeh) then
        ...
        ShowNotification("Falha ao encontrar um veículo.")
    end
end
